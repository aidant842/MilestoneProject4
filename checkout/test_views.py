...
"""     def test_post_order_detail_view(self):
        self.client.force_login(self.adminUser)
        order_form = OrderForm({
            'full_name': 'Adam Lee',
            'email': 'test@test.com',
        ...
        profile = UserProfile.objects.get(user=self.testUser)
        ...
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(len(messages), 1)
        self.assertEqual(str(messages[0]), 'Order updated successfully.') """


"""     def test_cache_checkout_view(self):
        self.client.force_login(self.adminUser)
        session = self.client.session
        session['bag'] = self.bag_with_item
        session.save()
        ...
        response = self.client.post('/checkout/cache_checkout_data/', data)
